...
def generate_hash_rows(src_client_db_path, src_user_db_path, table_name, result_query):
    ...
    engine_client_db._metadata.tables[table_name].columns = [
        i for i in engine_client_db._metadata.tables[table_name].columns if (i.name in client_columns_list)]
    table_client_db = Table(table_name, engine_client_db._metadata)
    ...
    engine_user_db._metadata.tables[table_name].columns = [
        i for i in engine_user_db._metadata.tables[table_name].columns if (i.name in client_user_list)]
    table_user_db = Table(table_name, engine_user_db._metadata)

    raw_data = pd.DataFrame(result_query, columns=client_columns_list)
    print(raw_data)
 
    update_hash_column(engine_client_db, engine_user_db, table_client_db, table_user_db, table_name, raw_data)


def generate_hash_column(src_client_db_path, src_user_db_path, src_table):
    ...
    # Section of Client Database to run sql operation
    session_client_db = Session(engine_client_db) 

    # Section of User Database to run sql operation
    session_user_db = Session(engine_user_db) 
